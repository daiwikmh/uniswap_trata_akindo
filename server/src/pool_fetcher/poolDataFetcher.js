...
export class PoolDataFetcher {
  constructor(poolConfig) {
    if (!poolConfig) {
      throw new Error("PoolDataFetcher requires a pool configuration object.");
    }

    this.provider = getProvider();
    this.poolConfig = poolConfig;

    // Compute pool address for the given tokens + fee
    this.poolAddress = Pool.getAddress(
      this.poolConfig.token0,
      this.poolConfig.token1,
      this.poolConfig.fee
    );
